...
class Buddy extends _Plugin.default {
  ...
  buddyAccept(args, user) {
    ...
    if (!user.buddy.requests.includes(args.id)) return; // Remove request

    user.buddy.requests = user.buddy.requests.filter(item => item != args.id); // Add to recipient buddy list

    user.buddy.addBuddy(args.id, args.username); // Add to requester buddy list
