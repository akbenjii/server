...
export default class FindFourInstance extends WaddleInstance {
    ...
    placeCounter(args, user) {
        ...
        if (verticalMatch || horizontalMatch || diagonalMatch) {
            ...
            this.send('four_over', { winner: winner })
            ...
            for (let x in this.users) {
                if (this.users[x].data.id === winner) {
                    ...
                    this.users[x].send('end_ruffle_mingame', { coins: this.users[x].data.coins, game: "four", coinsEarned: 25, stamps: this.users[x].stamps.list })
                }
                else {
                    this.users[x].updateCoins(10)
                    this.users[x].send('end_ruffle_mingame', { coins: this.users[x].data.coins, game: "four", coinsEarned: 10, stamps: this.users[x].stamps.list })
                }
    ...
    remove(user) {
        ...
        for (let x in this.users) {
            this.users[x].send('end_ruffle_mingame', { coins: this.users[x].data.coins, game: "four", coinsEarned: 0, stamps: this.users[x].stamps.list })
            this.waddle.remove(this.users[x])
        }
        this.waddle.reset()
    }
