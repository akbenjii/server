...
class Buddy {
  constructor(user) {
    ...
    this.db = user.db; // Buddy list

    this.list = []; // Pending requests that the user has received
  ...
  async init(buddies) {
    for (let buddy of buddies) {
      ...
      let online = this.isOnline(user.id); // Online status here is only used on initial load or adding of a new buddy,
  ...
  addRequest(id, username) {
    if (this.user.data.id == id) return; // If user is ignored

    if (this.user.ignore.includes(id)) return; // If buddy already added

    if (this.includes(id)) return; // If request has already been received

    if (this.requests.includes(id)) return;
    this.requests.push(id);
    this.user.send('buddy_request', {
      id: id,
      username: username
    });
  }
