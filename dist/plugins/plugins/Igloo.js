...
class Igloo extends _Plugin.default {
  ...
  async updateIgloo(args, user) {
    ...
    igloo.flooring = 0; // Refresh igloo
  ...
  async updateFurniture(args, user) {
    ...
    for (let item of args.furniture) {
      ...
      quantities[id] = quantities[id] ? quantities[id] + 1 : 1; // Validate quantity
      ...
      this.db.userFurnitures.create({ ...item,
        userId: user.data.id
      });
    }
  }
